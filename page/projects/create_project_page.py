...
class CreateProjectPage(BasePage):
    ...
    def load(self, host):
        """访问新建项目页面"""
        full_url = url_handle(host, "/projects/deposit/new")
        self.visit(full_url)
    ...
    def choose_project_type_language(self, **kwargs):
        ...
        self.click((pattern, locator.format(kwargs.get("project_category"))))
        allure_step(step_title=f"选择项目类别：{kwargs.get('project_category')}")
        ...
        self.click((pattern, locator.format(kwargs.get("project_language"))))
        allure_step(step_title=f"选择项目语言：{kwargs.get('project_language')}")
        time.sleep(2)
    ...
    def new_project(self, **kwargs):
        ...
        self.click((pattern, locator.format(kwargs.get("ignore"))))
        allure_step(step_title=f"选择gitignore：{kwargs.get('ignore')}")

        # 选择开源许可证
        self.click(certificate_checkbox)
        self.click(certificate_selectbox)
        pattern, locator = certificate_value
        self.click((pattern, locator.format(kwargs.get("license"))))
        allure_step(step_title=f"选择开源许可证：{kwargs.get('license')}")
