...
class TestLogin:
    ...
    def test_login_pop(self, case, init_driver):
        """
        ...
        allure_step(step_title=f"当前运行的浏览器驱动是：{init_driver}")
        logger.debug(f"当前运行的浏览器驱动是：{init_driver}")

        init_driver.delete_all_cookies()
        allure_step(step_title="清除浏览器缓存")

        full_url = ProjectsPage(init_driver).load(host)
        allure_step(step_title=f"访问：{full_url}")
        time.sleep(5)

        HomePage(init_driver).click_login_button()
        allure_step(step_title=f"游客状态下 点击 右上角 登录 按钮")

        LoginPop(init_driver).input_login_info(case["user"], case["password"])
        allure_step(step_title=f"输入--> 用户名: {case['user']}  密码：{case['password']}")

        LoginPop(init_driver).submit_login()
        allure_step(step_title=f"点击登录按钮，提交登录表单")

        # 断言
        expected_url = full_url
        logger.debug(f"断言--> 浏览器地址是否一致----预期：{expected_url} 实际：{init_driver.current_url}")
        assert expected_url == init_driver.current_url
        allure_step(step_title=f"断言--> 浏览器地址是否一致----预期：{expected_url} 实际：{init_driver.current_url}")
    ...
    def test_login_page(self, case, init_driver):
        """
        ...
        full_url = HomePage(init_driver).load(host)
        allure_step(step_title=f"访问：{full_url}")
        time.sleep(5)
        ...
        LoginPage(init_driver).input_login_info(case["user"], case["password"])
        allure_step(step_title=f"输入--> 用户名: {case['user']}  密码：{case['password']}")

        LoginPage(init_driver).submit_login()
        allure_step(step_title=f"点击登录按钮，提交登录表单")
